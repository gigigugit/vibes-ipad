"""DashboardWindow — UI-file based dashboard launched from MainWindow.

This module provides a ``DashboardWindow`` class whose layout is defined
by the ``.ui`` files in ``emr_assist/ui/designer/``.  It can be launched
from the "Open New Dashboard" button on the Dashboard tab of the old
tabbed MainWindow, *or* as a standalone application.

The ``.ui`` files are the canonical source for the dashboard layout so
that they can be edited visually in Qt Design Studio.  Business logic
lives entirely in Python and is wired after the UI loads.

Launch standalone:
    python -m emr_assist.ui.dashboard_window
"""
...
from emr_assist.ui.theme import (
    DARK_THEME_QSS,
    ACCENT_BLUE,
    ACCENT_GREEN,
    ACCENT_RED,
...
from emr_assist.ui.ui_loader import load_ui, get_ui_path
...
# ═══════════════════════════════════════════════════════════════════════════
# DashboardWindow — .ui-file-driven dashboard
# ═══════════════════════════════════════════════════════════════════════════
class DashboardWindow(QMainWindow):
    """Dockable-panel EMR dashboard whose main layout comes from .ui files.

    The central widget layout is loaded from ``dashboard_main.ui``.
    Dock widgets are built programmatically because they contain custom
    Python widgets whose constructors require arguments not expressible
    in .ui XML.  The ``.ui`` files for panels exist as reference layouts
    editable in Qt Design Studio; they are loaded to set up named
    placeholders that are then replaced or populated with the real panel
    widgets.
    """

    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
        state.panel_title = "EMR Dashboard v2"
        ...
        # ── Build UI from .ui files + code ────────────────────────────
        self._build_central_from_ui()
    ...
    # ==================================================================
    # UI Construction — central widget loaded from .ui
    # ==================================================================

    def _build_central_from_ui(self) -> None:
        """Load the central widget layout from dashboard_main.ui.

        The .ui file defines the status-bar area (visit badges, medication
        label, visit-type quick-switch chips).  After loading, we locate
        the named widgets and wire them up.
        """
        ui_path = get_ui_path("dashboard_main.ui")
        if not os.path.exists(ui_path):
            # Fallback: build programmatically (same as original Dashboard)
            self._build_status_bar_fallback()
            return

        try:
            central = load_ui("dashboard_main.ui")
        except Exception as exc:
            print(f"[DashboardWindow] Failed to load dashboard_main.ui: {exc}")
            self._build_status_bar_fallback()
            return

        self.setCentralWidget(central)

        # Locate named widgets set in the .ui file
        self.badge_visit = central.findChild(QWidget, "badge_visit")
        self.badge_connection = central.findChild(QWidget, "badge_connection")
        self.lbl_medication = central.findChild(QLabel, "lbl_medication")
        self.lbl_status = central.findChild(QLabel, "lbl_status")

        # Style the medication label
        if self.lbl_medication:
            self.lbl_medication.setStyleSheet(
                f"color: {ACCENT_GREEN}; font-size: 12px; font-weight: bold;"
            )
        if self.lbl_status:
            self.lbl_status.setStyleSheet("color: #a0b0c0; font-size: 11px;")

        # Replace placeholder badge widgets with real StatusBadge widgets
        # (The .ui file uses promoted StatusBadge, but constructors differ)
        self._replace_badge("badge_visit", "T Deficiency", ACCENT_BLUE, central)
        self._replace_badge("badge_connection", "Chrome: ?", "#888", central)

        # Wire visit-type quick-switch buttons
        self._visit_btns: List[ActionButton] = []
        btn_names = [
            "btn_visit_td", "btn_visit_hair", "btn_visit_photo",
            "btn_visit_sh", "btn_visit_pa", "btn_visit_bc",
        ]
        for i, name in enumerate(btn_names):
            btn = central.findChild(QWidget, name)
            if btn is not None:
                btn.clicked.connect(lambda checked, idx=i: self._switch_visit(idx))
                self._visit_btns.append(btn)

    def _replace_badge(self, obj_name: str, text: str, color: str, parent: QWidget) -> None:
        """Replace a placeholder widget with a real StatusBadge in-place."""
        placeholder = parent.findChild(QWidget, obj_name)
        if placeholder is None:
            return
        badge = StatusBadge(text, color)
        layout = placeholder.parentWidget().layout() if placeholder.parentWidget() else None
        if layout is not None:
            ...
            if idx >= 0:
                ...
                layout.insertWidget(idx, badge)
                setattr(self, obj_name, badge)
                return
        # Fallback: just set as attribute
        setattr(self, obj_name, badge)

    def _build_status_bar_fallback(self) -> None:
        """Programmatic fallback if .ui file can't be loaded."""
        central = QWidget()
        self.setCentralWidget(central)
    ...
    # ==================================================================
    # Dock widgets — these use Python panel classes directly
    # ==================================================================
...
def main() -> None:
    ...
    app.setApplicationName("EMR Dashboard v2")
